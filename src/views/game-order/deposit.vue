<template>
  <el-card>
    <el-form :inline="true" :model="query" class="search-form">
      ...
      <el-form-item label="游戏ID"><el-input v-model="query.game_id" clearable style="width: 150px" /></el-form-item>
    ...
    <el-table :data="tableData" border stripe v-loading="loading">
      ...
      <el-table-column prop="game_id" label="游戏ID" width="150" />
...
import { getDepositOrders } from '@/api/game-order'
const loading = ref(false), tableData = ref([]), total = ref(0)
const query = ref({ page: 1, page_size: 20, user_id: '', game_id: '', status: null })
async function fetchData() {
  loading.value = true
  try {
    const params = {}; Object.entries(query.value).forEach(([k, v]) => { if (v !== '' && v !== null) params[k] = v })
    const res = await getDepositOrders(params); tableData.value = res.data.list; total.value = res.data.total
